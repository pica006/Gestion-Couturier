...
class PDFController:
    ...
    def _build_footer_lines(self, salon_id: Optional[str]) -> Optional[list]:
        ...
        try:
            ...
            logger.exception("Erreur construction pied de page PDF")
            return None
    ...
    def generer_pdf_commande(self, commande_data: Dict) -> Optional[str]:
        ...
        try:
            ...
            if commande_data.get('salon_id'):
                ...
                logger.debug("Salon ID récupéré depuis commande_data.")
            ...
            if not salon_id and self.db_connection and commande_data.get('couturier_id'):
                try:
                    ...
                    if result and result[0]:
                        salon_id = result[0]
                        logger.debug("Salon ID récupéré depuis couturier_id.")
                except Exception as e:
                    logger.warning("Erreur récupération salon_id depuis couturier_id: %s", e)
            
            # 3. Aucun fallback session ici (controller sans dépendance Streamlit)
            ...
            if self.db_connection and salon_id:
                try:
                    ...
                    if logo_data and logo_data.get('logo_data'):
                        ...
                        logger.debug("Logo filigrane chargé depuis la base.")
                except Exception as e:
                    logger.warning("Erreur récupération logo filigrane depuis BDD: %s", e)
            ...
            def dessiner_filigrane(canvas_obj, doc_obj):
                ...
                if logo_filigrane_data:
                    try:
                        ...
                        logger.debug("Filigrane chargé depuis la BDD.")
                    except Exception as e:
                        logger.warning("Erreur chargement logo filigrane depuis BDD: %s", e)
                ...
                try:
                    ...
                    logger.warning("Erreur dessin filigrane: %s", e)
            ...
            def dessiner_footer(canvas_obj, doc_obj):
                ...
                try:
                    ...
                    logger.warning("Erreur dessin pied de page PDF commande: %s", e)
            ...
            if self.db_connection and salon_id:
                try:
                    ...
                    if logo_data and logo_data.get('logo_data'):
                        ...
                        logger.debug("Logo PDF chargé depuis la base.")
                except Exception as e:
                    logger.warning("Erreur récupération logo depuis BDD: %s", e)
            ...
            if logo_image:
                try:
                    ...
                    logger.warning("Erreur ajout logo PDF: %s", e)
            else:
                logger.info("Logo PDF non trouvé (BDD/fichiers).")
            ...
            if fabric_image_path and os.path.exists(fabric_image_path):
                try:
                    ...
                    logger.debug("Image tissu chargée depuis chemin.")
                except Exception:
                    logger.exception("Erreur chargement image tissu depuis chemin")
                    images_row.append(Paragraph("Image du tissu\nnon disponible", styles['Normal']))
            else:
                # Fallback production-safe : utiliser l'image binaire si le chemin n'existe plus.
                fabric_image_bytes = commande_data.get("fabric_image")
                if fabric_image_bytes:
                    try:
                        fabric_img = Image(io.BytesIO(fabric_image_bytes), width=7*cm, height=7*cm)
                        images_row.append(fabric_img)
                    except Exception:
                        logger.exception("Erreur chargement image tissu depuis binaire")
                        images_row.append(Paragraph("Image du tissu\nnon disponible", styles['Normal']))
                else:
                    images_row.append(Paragraph("Image du tissu\nnon disponible", styles['Normal']))
            ...
            if model_image_path and os.path.exists(model_image_path):
                try:
                    ...
                    logger.debug("Image modèle chargée depuis chemin.")
                except Exception:
                    logger.exception("Erreur chargement image modèle depuis chemin")
                    images_row.append(Paragraph("Image du modèle\nnon disponible", styles['Normal']))
            else:
                # Fallback production-safe : utiliser l'image binaire si le chemin n'existe plus.
                model_image_bytes = commande_data.get("model_image")
                if model_image_bytes:
                    try:
                        model_img = Image(io.BytesIO(model_image_bytes), width=7*cm, height=7*cm)
                        images_row.append(model_img)
                    except Exception:
                        logger.exception("Erreur chargement image modèle depuis binaire")
                        images_row.append(Paragraph("Image du modèle\nnon disponible", styles['Normal']))
                else:
                    images_row.append(Paragraph("Image du modèle\nnon disponible", styles['Normal']))
            ...
            with tempfile.NamedTemporaryFile(prefix="qr_", suffix=".png", delete=False) as tmp_qr:
                qr_path = tmp_qr.name
            qr_img.save(qr_path)
            ...
            try:
                ...
                logger.exception("Erreur suppression QR temporaire")

            logger.info("PDF commande généré avec succès.")
            return filepath

        except Exception:
            logger.exception("Erreur génération PDF commande")
            error_msg = "Erreur de génération du PDF."
            # Stocker l'erreur dans un attribut pour que la vue puisse y accéder
            self.last_error = error_msg
            self.last_error_details = "Consultez les logs serveur."
            return None
        ...
        try:
            ...
            if not salon_id and self.db_connection and commande_data.get('couturier_id'):
                try:
                    ...
                    if result and result[0]:
                        salon_id = result[0]
                        logger.debug("Salon ID récupéré pour PDF livraison.")
                except Exception as e:
                    logger.warning("Erreur récupération salon_id PDF livraison: %s", e)
            ...
            if self.db_connection and salon_id:
                try:
                    ...
                    if logo_data and logo_data.get('logo_data'):
                        logo_filigrane_data = logo_data['logo_data']
                        logger.debug("Logo filigrane livraison chargé depuis app_logo.")
                except Exception as e:
                    logger.warning("Erreur récupération logo livraison depuis app_logo: %s", e)
            ...
            if not logo_filigrane_data and salon_id and self.db_connection:
                try:
                    ...
                    if result and result[0]:
                        ...
                        logger.debug("Logo filigrane livraison chargé depuis fallback salons.logo.")
                except Exception as e:
                    logger.warning("Erreur récupération logo fallback livraison: %s", e)
            ...
            def dessiner_filigrane(canvas_obj, doc_obj):
                if logo_filigrane_data:
                    try:
                        ...
                        logger.warning("Erreur dessin filigrane livraison: %s", e)
            ...
            def dessiner_footer(canvas_obj, doc_obj):
                ...
                try:
                    ...
                    logger.warning("Erreur dessin pied de page PDF livraison: %s", e)
            ...
            logger.info("PDF de livraison généré avec succès.")
            return filepath

        except Exception:
            logger.exception("Erreur génération PDF livraison")
            error_msg = "Erreur de génération du PDF de livraison."
            self.last_error = error_msg
            self.last_error_details = "Consultez les logs serveur."
            return None