...
def afficher_vue_ensemble(super_admin_ctrl, salon_model):
    ...
    # Diagnostic UI sans accès SQL direct dans la vue
    if not salons:
        ...
        with st.expander("🔍 Debug - Diagnostic complet"):
            ...
            st.info("Connexion DB active mais aucun salon retourné par le modèle.")
            st.info("Vérifiez les données via la page de gestion des salons ou les scripts de seed.")
            st.markdown("---")
            st.caption("Aucun SQL direct exécuté depuis la vue (conforme MVC).")
        return
            ...
            if submitted:
                ...
                if not all(champs_obligatoires):
                    ...
                    try:
                        ...
                        logger.exception("Erreur lors de la création du salon")
                ...
                with st.form(f"form_valider_super_{demande['id']}", clear_on_submit=True):
                    ...
                    if st.form_submit_button("✅ Valider", type="primary"):
                        try:
                            ...
                            logger.exception("Erreur lors de la validation de demande super admin")
                            st.error("❌ Erreur lors de la validation.")
                ...
                with st.form(f"form_rejeter_super_{demande['id']}", clear_on_submit=True):
                    ...
                    if st.form_submit_button("❌ Rejeter"):
                        try:
                            ...
                            logger.exception("Erreur lors du rejet de demande super admin")
                            st.error("❌ Erreur lors du rejet.")
