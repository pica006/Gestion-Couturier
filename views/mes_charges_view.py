...
def calculer_prochaine_reference(charges_model: ChargesModel, couturier_id: int, salon_id: Optional[str] = None) -> int:
    ...
    try:
        ...
        logger.warning("Erreur calcul prochaine référence: %s", e)
        return 1
...
def sauvegarder_fichier_charge(uploaded_file, charge_id: int) -> Optional[Dict]:
    ...
    try:
        ...
        logger.exception("Erreur lors de la sauvegarde du fichier justificatif")
    ...
    try:
        ...
        if salon_id and st.session_state.get('db_connection'):
            ...
            if logo_data and logo_data.get('logo_data'):
                logger.debug("Logo récupéré depuis la BDD.")
...
def _generer_pdf_impots(date_debut,
    ...
    try:
        ...
        try:
            ...
            logger.warning("Erreur construction pied de page impôts: %s", e)
        ...
        def dessiner_filigrane(canvas_obj, doc_obj):
            ...
            if logo_filigrane_data:
                try:
                    ...
                    logger.debug("Filigrane impôts chargé depuis la BDD.")
                except Exception as e:
            ...
            try:
                ...
                logger.warning("Erreur filigrane impôts: %s", e)
        ...
        def dessiner_footer(canvas_obj, doc_obj):
            ...
            try:
                ...
                logger.warning("Erreur pied de page impôts: %s", e)
        ...
        if logo_filigrane_data:
            try:
                ...
                logger.warning("Erreur logo impôts (BDD): %s", e)
        ...
        logger.exception("Erreur génération PDF impôts")
        return None
...
def _generer_pdf_analyse_charges(date_debut,
    ...
    try:
        ...
        try:
            ...
            logger.warning("Erreur construction pied de page analyse charges: %s", e)
        ...
        def dessiner_filigrane(canvas_obj, doc_obj):
            ...
            if logo_filigrane_data:
                try:
                    logo_img = PILImage.open(io.BytesIO(logo_filigrane_data))
                    logger.debug("Filigrane analyse charges chargé depuis la BDD.")
                except Exception as e:
                    logger.warning("Erreur chargement logo filigrane depuis BDD: %s", e)
            ...
            try:
                ...
                logger.warning("Erreur filigrane analyse charges: %s", e)
        ...
        if logo_filigrane_data:
            try:
                ...
                logger.warning("Erreur logo analyse charges (BDD): %s", e)
        ...
        def dessiner_footer(canvas_obj, doc_obj):
            ...
            try:
                ...
                logger.warning("Erreur pied de page analyse charges: %s", e)
        ...
        try:
            ...
            logger.warning("Erreur génération graphique analyse charges: %s", e)
        ...
        logger.exception("Erreur génération PDF analyse charges")
        return None
...
def _generer_pdf_bulletin_salaire(employe_nom: str,
    ...
    try:
        ...
        try:
            ...
            logger.warning("Erreur construction pied de page bulletin salaire: %s", e)
        ...
        def dessiner_filigrane(canvas_obj, doc_obj):
            ...
            if logo_filigrane_data:
                try:
                    logo_img = PILImage.open(io.BytesIO(logo_filigrane_data))
                    logger.debug("Filigrane bulletin salaire chargé depuis la BDD.")
                except Exception as e:
                    logger.warning("Erreur chargement logo filigrane depuis BDD: %s", e)
            ...
            try:
                ...
                logger.warning("Erreur filigrane bulletin salaire: %s", e)
        ...
        try:
            ...
            logger.warning("Erreur QR code bulletin salaire: %s", e)
        ...
        def dessiner_footer(canvas_obj, doc_obj):
            ...
            try:
                ...
                logger.warning("Erreur pied de page bulletin salaire: %s", e)
        ...
        logger.exception("Erreur génération bulletin salaire")
        return None
