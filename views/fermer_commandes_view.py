...
def afficher_page_fermer_commandes():
    ...
    fermeture_controller = FermetureController(db)
    ...
    with tab1:
        ...
        try:
            commandes_avec_reste = fermeture_controller.lister_commandes_avec_reste(
                        ...
                        if submit:
                            ...
                            if nouvelle_avance_ajoutee > reste_a_verser:
                                ...
                                with st.spinner("💾 Enregistrement des modifications..."):
                                    ...
                                    success = fermeture_controller.enregistrer_modification_paiement(
                                        commande_id=commande['id'],
                                        prix_total=nouveau_prix_total,
                                        avance=nouvelle_avance_totale,
                                        reste=nouveau_reste,
                                    )
                                    if success and nouveau_reste <= 0:
    ...
    with tab2:
        ...
        try:
            couturier_cible = couturier_id_filter if is_admin_user else couturier_id
            commandes_terminees = fermeture_controller.lister_commandes_terminees(
                salon_id=salon_id_user,
                couturier_id=couturier_cible,
                date_debut=date_debut_terminees,
                date_fin=date_fin_terminees,
                statut="Terminé",
            )

            demandes_map = fermeture_controller.demandes_en_attente_map()
            historique_counts = {}
            if not is_admin_user and commandes_terminees:
                ids = [cmd["id"] for cmd in commandes_terminees]
                historique_counts = fermeture_controller.demandes_stats_par_commandes(couturier_id, ids)

            for cmd in commandes_terminees:
                cmd["demande_existante"] = demandes_map.get(cmd["id"])
        ...
        if not commandes_terminees:
            ...
            for commande in commandes_terminees:
                ...
                with st.expander(
                    ...
                    resume_demande = fermeture_controller.demande_resume_commande(
                        commande_id=commande["id"],
                        couturier_id=couturier_id,
                    )
                    total_demandes = resume_demande.get("total", 0)
                    derniere_demande_status = resume_demande.get("last_status")
                    ...
                    if is_admin_user:
                        ...
                        if st.button(
                            ...
                            try:
                                fermeture_controller.valider_livraison(commande["id"])
    ...
    with tab3:
        ...
        # Récupérer les commandes validées pour téléchargement PDF
        try:
            couturier_cible = couturier_id_filter if is_admin_user else couturier_id
            commandes_terminees = fermeture_controller.lister_commandes_terminees(
                salon_id=salon_id_user,
            ...
            if nom_client_filter:
                needle = nom_client_filter.lower()
                commandes_terminees = [
                    c for c in commandes_terminees
                    if needle in str(c.get("client_nom", "")).lower()
                    or needle in str(c.get("client_prenom", "")).lower()
                ]
        except Exception:
            st.error("❌ Erreur lors de la récupération des commandes terminées.")
            commandes_terminees = []
