    ...
    if wallpaper_path:
        ...
        if os.path.exists(image_path):
            try:
                ...
                st.warning("⚠️ Impossible de charger l'image de fond.")
        else:
            st.warning(f"⚠️ Image de fond introuvable : {image_path}")
    
    # Connexion DB orchestree par app.py (via services/database_service.py).
    db_ready = bool(
            ...
            if submit_auth:
                ...
                if not code_couturier:
                    ...
                    with st.spinner("Vérification des identifiants..."):
                        ...
                        if not db_ready:
                            st.error("❌ Base de données indisponible. Réessayez dans un instant.")
                            return

                        # Auth applicative via service (la vue reste orchestrateur UI).
                        succes, donnees, message, page_cible = authenticate_user(
                        ...
                        if succes:
                            ...
                            # Redirection selon le rôle (service d'auth)
                            st.session_state.page = page_cible
